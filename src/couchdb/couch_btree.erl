...
-record(btree_stats, {
    ...
    min_reduction_size = infinity,
    max_elements_per_kp_node = 0,
    min_elements_per_kp_node = infinity,
    avg_elements_per_kp_node = 0,
    max_elements_per_kv_node = 0,
    min_elements_per_kv_node = infinity,
    avg_elements_per_kv_node = 0,
    kv_count = 0,
    min_kp_node_size = infinity,
    min_kp_node_size_compressed = infinity,
    max_kp_node_size = 0,
    max_kp_node_size_compressed = 0,
    avg_kp_node_size = 0,
    avg_kp_node_size_compressed = 0,
    min_kv_node_size = infinity,
    min_kv_node_size_compressed = infinity,
    max_kv_node_size = 0,
    max_kv_node_size_compressed = 0,
    avg_kv_node_size = 0,
    avg_kv_node_size_compressed = 0
}).
...
stats(#btree{root = Root, fd = Fd} = Bt) ->
    #btree_stats{
        kv_nodes = KvNodeCount,
        kp_nodes = KpNodeCount,
        avg_elements_per_kv_node = AvgKvNodeSum,
        avg_elements_per_kp_node = AvgKpNodeSum,
        avg_kv_node_size = AvgKvNodeSize,
        avg_kv_node_size_compressed = AvgKvNodeSizeCompressed,
        avg_kp_node_size = AvgKpNodeSize,
        avg_kp_node_size_compressed = AvgKpNodeSizeCompressed
    } = StatsRec = collect_stats(Root, Bt, 0, 0, 0, 0, 0, 0),
    StatsRec2 = case KvNodeCount of
    0 ->
        StatsRec;
    _ ->
        StatsRec#btree_stats{
            avg_elements_per_kv_node = AvgKvNodeSum / KvNodeCount,
            avg_kv_node_size = AvgKvNodeSize / KvNodeCount,
            avg_kv_node_size_compressed = AvgKvNodeSizeCompressed / KvNodeCount
        }
    end,
    StatsRec3 = case KpNodeCount of
    0 ->
        StatsRec2;
    _ ->
        StatsRec2#btree_stats{
            avg_elements_per_kp_node = AvgKpNodeSum / KpNodeCount,
            avg_kp_node_size = AvgKpNodeSize / KpNodeCount,
            avg_kp_node_size_compressed = AvgKpNodeSizeCompressed / KpNodeCount
        }
    end,
    StatNames = record_info(fields, btree_stats),
    StatPoses = lists:seq(2, record_info(size, btree_stats)),
    Stats = lists:foldr(
        fun({StatName, StatPos}, Acc) ->
            case element(StatPos, StatsRec3) of
            infinity ->
                Value = 0;
            Value ->
                ok
    ...
    TreeSize = ?MODULE:size(Bt),
    Frag = case FileSize of
    0 ->
        0;
    _ ->
        ((FileSize - TreeSize) / FileSize) * 100
    end,
    [
        {btree_size, TreeSize},
        {file_size, FileSize},
...
collect_stats(nil, _Bt, 0, 0, 0, 0, 0, 0) ->
    #btree_stats{};
collect_stats({Pointer, Reduction, _Size}, Bt, KvElemCountAvg, KpElemCountAvg,
        KvSizeAvg, KvSizeCompAvg, KpSizeAvg, KpSizeCompAvg) ->
    {{NodeType, NodeList}, NodeCompSize, NodeSize} = stats_get_node(Bt, Pointer),
    ChildCount = length(NodeList),
    RedSize = case is_binary(Reduction) of
    true ->
        byte_size(Reduction);
    false ->
        ?term_size(Reduction)
    ...
    kv_node ->
        #btree_stats{
            ...
            min_depth = 1,
            kv_count = ChildCount,
            min_elements_per_kv_node = ChildCount,
            max_elements_per_kv_node = ChildCount,
            avg_elements_per_kv_node = KvElemCountAvg + ChildCount,
            avg_elements_per_kp_node = KpElemCountAvg,
            min_reduction_size = RedSize,
            max_reduction_size = RedSize,
            min_kv_node_size = NodeSize,
            min_kv_node_size_compressed = NodeCompSize,
            max_kv_node_size = NodeSize,
            max_kv_node_size_compressed = NodeCompSize,
            avg_kv_node_size = KvSizeAvg + NodeSize,
            avg_kv_node_size_compressed = KvSizeCompAvg + NodeCompSize,
            avg_kp_node_size = KpSizeAvg,
            avg_kp_node_size_compressed = KpSizeCompAvg
        };
    kp_node ->
        Stats0 = #btree_stats{
            min_reduction_size = RedSize,
            max_reduction_size = RedSize,
            avg_elements_per_kv_node = KvElemCountAvg,
            avg_elements_per_kp_node = KpElemCountAvg + ChildCount,
            min_kp_node_size = NodeSize,
            min_kp_node_size_compressed = NodeCompSize,
            max_kp_node_size = NodeSize,
            max_kp_node_size_compressed = NodeCompSize,
            avg_kp_node_size = KpSizeAvg + NodeSize,
            avg_kp_node_size_compressed = KpSizeCompAvg + NodeCompSize,
            avg_kv_node_size = KvSizeAvg,
            avg_kv_node_size_compressed = KvSizeCompAvg
        },
        Stats = lists:foldl(
            fun({_Key, NodeState}, StatsAcc) ->
                SubStats = collect_stats(
                    NodeState, Bt,
                    StatsAcc#btree_stats.avg_elements_per_kv_node,
                    StatsAcc#btree_stats.avg_elements_per_kp_node,
                    StatsAcc#btree_stats.avg_kv_node_size,
                    StatsAcc#btree_stats.avg_kv_node_size_compressed,
                    StatsAcc#btree_stats.avg_kp_node_size,
                    StatsAcc#btree_stats.avg_kp_node_size_compressed),
                StatsAcc#btree_stats{
                    kp_nodes = StatsAcc#btree_stats.kp_nodes + SubStats#btree_stats.kp_nodes,
                    kv_nodes = StatsAcc#btree_stats.kv_nodes + SubStats#btree_stats.kv_nodes,
                    kv_count = StatsAcc#btree_stats.kv_count + SubStats#btree_stats.kv_count,
                    max_depth = erlang:max(
                        StatsAcc#btree_stats.max_depth, SubStats#btree_stats.max_depth),
                    min_depth = erlang:min(
                        StatsAcc#btree_stats.min_depth, SubStats#btree_stats.min_depth),
                    avg_elements_per_kp_node = SubStats#btree_stats.avg_elements_per_kp_node,
                    min_elements_per_kp_node = erlang:min(
                        StatsAcc#btree_stats.min_elements_per_kp_node,
                        SubStats#btree_stats.min_elements_per_kp_node),
                    max_elements_per_kp_node = erlang:max(
                        StatsAcc#btree_stats.max_elements_per_kp_node,
                        SubStats#btree_stats.max_elements_per_kp_node),
                    avg_elements_per_kv_node = SubStats#btree_stats.avg_elements_per_kv_node,
                    min_elements_per_kv_node = erlang:min(
                        StatsAcc#btree_stats.min_elements_per_kv_node,
                        SubStats#btree_stats.min_elements_per_kv_node),
                    max_elements_per_kv_node = erlang:max(
                        StatsAcc#btree_stats.max_elements_per_kv_node,
                        SubStats#btree_stats.max_elements_per_kv_node),
                    min_reduction_size = erlang:min(
                        StatsAcc#btree_stats.min_reduction_size,
                        SubStats#btree_stats.min_reduction_size),
                    max_reduction_size = erlang:max(
                        StatsAcc#btree_stats.max_reduction_size,
                        SubStats#btree_stats.max_reduction_size),
                    min_kv_node_size = erlang:min(
                        StatsAcc#btree_stats.min_kv_node_size,
                        SubStats#btree_stats.min_kv_node_size),
                    min_kv_node_size_compressed = erlang:min(
                        StatsAcc#btree_stats.min_kv_node_size_compressed,
                        SubStats#btree_stats.min_kv_node_size_compressed),
                    max_kv_node_size = erlang:max(
                        StatsAcc#btree_stats.max_kv_node_size,
                        SubStats#btree_stats.max_kv_node_size),
                    max_kv_node_size_compressed = erlang:max(
                        StatsAcc#btree_stats.max_kv_node_size_compressed,
                        SubStats#btree_stats.max_kv_node_size_compressed),
                    avg_kv_node_size = SubStats#btree_stats.avg_kv_node_size,
                    avg_kv_node_size_compressed = SubStats#btree_stats.avg_kv_node_size_compressed,
                    min_kp_node_size = erlang:min(
                        StatsAcc#btree_stats.min_kp_node_size,
                        SubStats#btree_stats.min_kp_node_size),
                    min_kp_node_size_compressed = erlang:min(
                        StatsAcc#btree_stats.min_kp_node_size_compressed,
                        SubStats#btree_stats.min_kp_node_size_compressed),
                    max_kp_node_size = erlang:max(
                        StatsAcc#btree_stats.max_kp_node_size,
                        SubStats#btree_stats.max_kp_node_size),
                    max_kp_node_size_compressed = erlang:max(
                        StatsAcc#btree_stats.max_kp_node_size_compressed,
                        SubStats#btree_stats.max_kp_node_size_compressed),
                    avg_kp_node_size = SubStats#btree_stats.avg_kp_node_size,
                    avg_kp_node_size_compressed = SubStats#btree_stats.avg_kp_node_size_compressed
                }
            end,
            Stats0,
            NodeList),
        Stats#btree_stats{
            kp_nodes = Stats#btree_stats.kp_nodes + 1,
            min_depth = Stats#btree_stats.min_depth + 1,
            max_depth = Stats#btree_stats.max_depth + 1,
            min_elements_per_kp_node = erlang:min(
                Stats#btree_stats.min_elements_per_kp_node, ChildCount),
            max_elements_per_kp_node = erlang:max(
                Stats#btree_stats.max_elements_per_kp_node, ChildCount)
        }
    end.


stats_get_node(#btree{fd = Fd, binary_mode = false}, NodePos) ->
    {ok, CompressedBin} = couch_file:pread_binary(Fd, NodePos),
    Bin = couch_compress:decompress(CompressedBin),
    {binary_to_term(Bin), byte_size(CompressedBin), byte_size(Bin)};
stats_get_node(#btree{fd = Fd, binary_mode = true}, NodePos) ->
    {ok, CompressedBin} = couch_file:pread_binary(Fd, NodePos),
    <<TypeInt, NodeBin/binary>> = Bin = couch_compress:decompress(CompressedBin),
    Type = if TypeInt == 1 -> kv_node; true -> kp_node end,
    {decode_node(Type, NodeBin, []), byte_size(CompressedBin), byte_size(Bin)}.
