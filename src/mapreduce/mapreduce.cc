...
static const char* BASE64_FUNCTION_STRING =
        ...
        "        tmp = (lookup.indexOf(b64[i]) << 18) | (lookup.indexOf(b64[i "
        "+ 1]) << 12);"
        "        tmp |= (lookup.indexOf(b64[i + 2]) << 6) | "
        ...
        "    }"
        "    if (scratch === 2) {"
        "        tmp = (lookup.indexOf(b64[i]) << 2) | (lookup.indexOf(b64[i + "
        "1]) >> 4);"
        "        arr.push(tmp & 0xFF);"
        "    } else if (scratch === 1) {"
        "        tmp = (lookup.indexOf(b64[i]) << 10) | (lookup.indexOf(b64[i "
        "+ 1]) << 4);"
...
void initContext(map_reduce_ctx_t* ctx,
                 const function_sources_list_t& funs,
                 const view_index_type_t viewType) {
...
static Local<String> createUtf8String(Isolate* isolate,
                                      const char* str,
                                      size_t len) {
...
void doInitContext(map_reduce_ctx_t* ctx,
                   const function_sources_list_t& funs,
                   const view_index_type_t viewType) {
...
map_results_list_t mapDoc(map_reduce_ctx_t* ctx,
    ...
    for (unsigned int i = 0; i < ctx->functions->size(); ++i) {
        ...
        if (result.IsEmpty()) {
            ...
            if (!enif_alloc_binary_compat(
                        ctx->env, len, mapResult.result.error)) {
                freeMapResultList(results);
                throw std::bad_alloc();
            }
...
ErlNifBinary runReduce(map_reduce_ctx_t* ctx,
    ...
    Local<Function> fun = Local<Function>::New(
            ctx->isolate, *(*ctx->functions)[reduceFunNum]);
    Handle<Array> keysArray = jsonListToJsArray(keys);
    Handle<Array> valuesArray = jsonListToJsArray(values);
    Handle<Value> args[] = {
            keysArray, valuesArray, Boolean::New(ctx->isolate, false)};
...
static Local<Context> createJsContext(map_reduce_ctx_t* ctx) {
    ...
    CHECK_SUCCESS(context->Global()->Set(
            context, createUtf8String(ctx->isolate, "sum"), sumFun));

    Handle<Function> decodeBase64Fun = compileFunction(BASE64_FUNCTION_STRING);
    CHECK_SUCCESS(context->Global()->Set(
            context,
            createUtf8String(ctx->isolate, "decodeBase64"),
            decodeBase64Fun));

    Handle<Function> dateToArrayFun = compileFunction(DATE_FUNCTION_STRING);
    CHECK_SUCCESS(context->Global()->Set(
            context,
            createUtf8String(ctx->isolate, "dateToArray"),
            dateToArrayFun));
...
static void log(const v8::FunctionCallbackInfo<Value>& args) {
    ...
    try {
        ...
        if (ctx->logResults == NULL) {
            ctx->logResults =
                    (log_results_list_t*)enif_alloc(sizeof(log_results_list_t));
            if (ctx->logResults == NULL) {
                throw std::bad_alloc();
            }
        ...
        if (logMsg->IsString()) {
            ...
            if (len > MAX_LOG_STRING_SIZE) {
                str = Handle<String>(String::Concat(
                        ctx->isolate,
                        createUtf8String(ctx->isolate, TRUNCATE_STR),
                        str)),
                len = MAX_LOG_STRING_SIZE + sizeof(TRUNCATE_STR) - 1;
            }
        } else {
            str = Handle<String>(createUtf8String(
                    ctx->isolate,
        ...
        str->WriteUtf8(ctx->isolate,
                       reinterpret_cast<char*>(resultBin.data),
                       len,
                       NULL,
...
static void emit(const v8::FunctionCallbackInfo<Value>& args) {
    ...
    if ((isoData->ctx->maxEmitKvSize > 0) &&
        ...
        std::stringstream msg;
        msg << "too much data emitted: " << isoData->ctx->emitKvSize
            << " bytes";
...
Handle<Value> jsonParse(const ErlNifBinary& thing) {
    ...
    Local<Context> context(isoData->ctx->isolate->GetCurrentContext());
    Handle<Value> args[] = {
            createUtf8String(isoData->ctx->isolate,
