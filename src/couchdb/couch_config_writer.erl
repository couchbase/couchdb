...
save_to_file({{Section, Option}, Value}, File) ->
    ...
    SectionLine = "[" ++ Section ++ "]",
    {ok, Pattern} = re:compile("^(" ++ Option ++ "\\s*=)|" ++
                               "\\[[a-zA-Z0-9\_-]*\\]"),

    NewLines = process_file_lines(Lines, [], SectionLine,
...
process_file_lines([Line|Rest], SeenLines,
                   SectionLine, Pattern, Option, Value) ->
    case Line of
        SectionLine ->
            process_section_lines(Rest, [Line|SeenLines],
                                  Pattern, Option, Value);
        _ ->
            process_file_lines(Rest, [Line|SeenLines],
                               SectionLine, Pattern, Option, Value)
    end;

process_file_lines([], SeenLines, SectionLine, _Pattern, Option, Value) ->
    % Section wasn't found.  Append it with the option here.
    [Option ++ " = " ++ Value, SectionLine, "" | strip_empty_lines(SeenLines)].


process_section_lines([Line|Rest], SeenLines, Pattern, Option, Value) ->
    case re:run(Line, Pattern, [{capture, all_but_first}]) of
        ...
        {match, []} -> % Found another section. Append the option here.
            lists:reverse(Rest) ++
            [Line, "", Option ++ " = " ++ Value | strip_empty_lines(SeenLines)];
        {match, _} -> % Found the option itself. Replace it.
            lists:reverse(Rest) ++ [Option ++ " = " ++ Value | SeenLines]
    end;

process_section_lines([], SeenLines, _Pattern, Option, Value) ->
    % Found end of file within the section. Append the option here.
    [Option ++ " = " ++ Value | strip_empty_lines(SeenLines)].
