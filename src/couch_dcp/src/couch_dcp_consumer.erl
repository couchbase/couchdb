...
-export([parse_header/1, parse_snapshot_marker/1, parse_snapshot_mutation/4,
    parse_snapshot_deletion/2, parse_failover_log/1, parse_stat/4, parse_advance_seqno/1, parse_system_event/1]).
-export([encode_sasl_auth/3, encode_hello_message/3, encode_open_connection/3, encode_stream_request/9,
...
parse_header(<<?DCP_MAGIC_REQUEST,
    ...
    ?DCP_SYSTEM_EVENT ->
        {system_event, PartId, RequestId, KeyLength, BodyLength, ExtraLength, Cas, DataType};
    ?DCP_ADVANCE_SEQ_NUMBER ->
...
-spec parse_advance_seqno(<<_:8>>) ->
                                   {seqno, update_seq()}.
parse_advance_seqno(Body) ->
    <<Seq:?DCP_SIZES_BY_SEQ>> = Body,
    {seqno, Seq}.

-spec parse_system_event(binary()) -> {system_event, update_seq()}.
parse_system_event(Body) ->
    <<Seq:?DCP_SIZES_BY_SEQ, _/binary>> = Body,
    {seqno, Seq}.

decode_leb128(Bin) when is_binary(Bin) ->
    decode_leb128(Bin, 0, 0).

% Helper function to decode recursively
decode_leb128(<<Byte:8, Rest/binary>>, Value, Shift) ->
    NewValue = Value bor ((Byte band 127) bsl Shift),
    if
        Byte band 128 =:= 0 ->
            {NewValue, Rest};
        true ->
            decode_leb128(Rest, NewValue, Shift + 7)
    end;
decode_leb128(<<>>, Value, _) ->
    {0, Value}.
...
-spec encode_stream_request(partition_id(), request_id(), non_neg_integer(),
                            ...
                            uuid(), update_seq(), update_seq(), string()) -> binary().
encode_stream_request(PartId, RequestId, Flags, StartSeq, EndSeq, PartUuid,
        SnapshotStart, SnapshotEnd, Cid) ->
    Value = list_to_binary("{\"collections\" : [\""++Cid++"\"]}"), %make it as json object
    ...
    BodyLength = byte_size(Body),
    ExtraLength = byte_size(Body)-byte_size(Value),
    Header = <<?DCP_MAGIC_REQUEST,
               ?DCP_OPCODE_STREAM_REQUEST,
