...
% to help simplify your view code.-module(couch_app_server_erlang).
-module(couch_app_server_erlang).
...
get_server(_Arg, DDoc, {DDocId, _}) ->
    {ok, Pid} = gen_server:start_link(?MODULE, [], []),
    Data = [<<"ddoc">>, <<"new">>, DDocId, DDoc],
    true = gen_server:call(Pid, {prompt, Data}),
    {ok, Pid}.

ret_server(Pid) ->
    gen_server:cast(Pid, stop).

show_doc(Pid, DDocId, ShowName, Doc, Req) ->
    Data = [<<"ddoc">>, DDocId, [<<"shows">>, ShowName], [Doc, Req]],
    [<<"resp">>, ExternalResp] = gen_server:call(Pid, {prompt, Data}),
    {ok, ExternalResp}.

list_start(Pid, DDocId, ListName, Head, Req) ->
    Data = [<<"ddoc">>, DDocId, [<<"lists">>, ListName], [Head, Req]],
    [<<"start">>, Chunks, JsonResp] = gen_server:call(Pid, {prompt, Data}),
    {ok, Chunks, JsonResp}.

list_row(Pid, _DDocId, Row) ->
    Data = [<<"list_row">>, Row],
    [Go, Chunks] = gen_server:call(Pid, {prompt, Data}),
    case Go of
        <<"chunks">> -> {ok, Chunks};
        <<"end">> -> {stop, Chunks}
    end.

list_end(Pid, _DDocId) ->
    [<<"end">>, Chunks] = gen_server:call(Pid, {prompt, [<<"list_end">>]}),
    {ok, Chunks}.

update_doc(Pid, DDocId, UpdateName, Doc, Req) ->
    Data = [<<"ddoc">>, DDocId, [<<"updates">>, UpdateName], [Doc, Req]],
    [<<"up">>, NewDoc, Resp] = gen_server:call(Pid, {prompt, Data}),
    {ok, NewDoc, Resp}.

validate_update(Pid, DDocId, EditDoc, DiskDoc, Context, SecObj) ->
    Data = [
        <<"ddoc">>,
        DDocId,
        [<<"validate_doc_update">>],
        [EditDoc, DiskDoc, Context, SecObj]
    ],
...
store_ddoc(DDocs, DDocId, DDoc) ->
    io:format("Storing: ~p~n", [DDocId]),
    dict:store(DDocId, DDoc, DDocs).
load_ddoc(DDocs, DDocId) ->
    io:format("Loading: ~p~n", [DDocId]),
